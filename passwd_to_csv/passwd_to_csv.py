...
暖身觀念 :
1. 讀 CSV 檔 : csv.reader 真正作用是是把每一個字串用指定的 delimiter 拆解成一個 list
2. 寫 CSV 檔 : csv.writer 真正作用是把每一個 list(或是 csv.reader 產生的), 用指定的 delimiter 和 lineterminator 串接回一個一個字串
...
current_p = os.getcwd()
file_p_read = os.path.join(current_p, 'passwd.cfg')
file_p_write = os.path.join(current_p, 'passwd.csv')
...
def passwd_to_csv(filename1, filename2):
    ...
    with open(filename1, 'r') as f_read, open(filename2, 'w', newline='') as f_write :
        ...
        for line in csv_reader:
            # writerow() 吃的參數要注意是 list, 若只是要寫入 list 的某幾個參數, 記得還是要用中括號 [] 括起來
            # csv_writer.writerow(line)
            csv_writer.writerow([ line[0], line[2] ])

passwd_to_csv(file_p_read, file_p_write)
